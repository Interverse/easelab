...
function applyOverviewFamily() {
  ...
  Object.values(groupMap).forEach((group) => {
    ...
    group.querySelector(".ease-group-head span").textContent = count + " types";
  });
...
["All", ...familyNames].forEach((name) => {
  ...
  button.title = name === "All" ? "Show every family" : "Toggle " + name;
  button.onclick = () => {
...
function renderEaseEditor() {
  ...
  $("channelEnabledLabel").textContent = hasIncoming ? "Enabled for Point " + (pointIndex + 1) : "Start point";
  $("selectedEase").textContent = (hasIncoming ? "Point " + (pointIndex + 1) + " · " : "Start · ") + "EaseType." + configEase(c);
...
let nextId = 3,
  ...
  history = [],
  historyIndex = -1;
document.querySelector(".hint").textContent = "Keep at least 2 points. Select one and press Delete to remove it.";
...
function renderList() {
  ...
  points.forEach((p, i) => {
    ...
    row.tabIndex = -1;
    row.innerHTML =
      '<span class="handle" title="Drag point to reorder">⠿</span><span class="index">' +
      (i + 1) +
      '</span><div class="point-fields">' +
      field("x", "X", Math.round(p.x)) +
      field("y", "Y", Math.round(p.y)) +
      field("scale", "Scale", Number(p.scale.toFixed(2))) +
      field("rotation", "Rotate °", Math.round(p.rotation)) +
      (i ? field("duration", "From prev ms", Math.round(pointDuration(p))) : "") +
      '</div><button class="button icon-btn danger" data-a="delete" aria-label="Delete point" title="Delete point" ' +
...
function renderMarkers() {
  ...
  points.forEach((p, i) => {
    ...
    m.style.top = p.y + "px";
    m.textContent = i + 1;
    m.setAttribute("aria-label", "Point " + (i + 1));
    m.onclick = (e) => {
      e.stopPropagation();
      selected = p.id;
      syncHistoryContext();
  ...
  if (p) {
    ...
    g.innerHTML =
      '<span class="gizmo-axis"></span><button class="gizmo-handle gizmo-rotate" type="button" title="Drag to rotate">R</button><button class="gizmo-handle gizmo-scale" type="button" title="Drag to scale">S</button>';
    box.append(g);
    bindGizmo(g, p);
  }
